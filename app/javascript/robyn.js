...
$(window).on("turbo:load", function () {
  ...
  // mirror the site name + section label into the mobile header bar (no nav menu there)
  const pageLabel =
    activeTab === "robyn-home"
      ? ""
      : $(`#${activeTab} .nav-link`).text().trim();
  const pageTitle = pageLabel ? `The Asking Tree - ${pageLabel}` : "";
  ...
  const init = function () {
    ...
    $(".typeahead").bind(
      ...
      function (event, suggestion, dataset) {
        console.log(
          "dataset: " + dataset + "; selected: " + suggestion.search_value,
          " - " + suggestion.id,
        );
